...
默认情况下，`scx_hpc` 使用 `--auto-mode smart`，会自动保留少量低容量的完整物理核心作为服务核心，其余核心作为计算核心：
...
| 选项 | 默认值 | 说明 |
|------|--------|------|
| `--compute-cpus <HEX>` | `0`（自动） | 计算核心的十六进制位掩码。`0` = 按 `--auto-mode` 自动选择。 |
| `--service-cpus <HEX>` | `0`（自动） | 服务核心的十六进制位掩码。`0` = 按 `--auto-mode` 自动选择。 |
| `--auto-mode <smart\|legacy>` | `smart` | 未手工指定 CPU 掩码时的自动分区策略。`smart` 会保留完整物理核心并尽量把服务能力分布到各 NUMA 节点；`legacy` 保持旧的“仅最低容量单 CPU”行为。 |
...
- **绑定任务例外**：迁移受限或 CPU 亲和性受限、且无法在任何服务核心上运行的 SERVICE 任务，仍可能短暂运行在计算核心上。
